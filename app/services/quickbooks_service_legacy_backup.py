...
class QuickBooksService:
    ...
    async def query(self, query_string: str) -> List[Any]:
        ...
        try:
            response = await self._make_request("GET", "query", params={"query": query_string})
            
            # Extract entity type from query (e.g., "SELECT * FROM Payment" -> "Payment")
            import re
            match = re.search(r'FROM\s+(\w+)', query_string, re.IGNORECASE)
            entity_type = match.group(1) if match else "Entity"
            
            # QB returns results in QueryResponse.<EntityType> format
            query_response = response.get("QueryResponse", {})
            results = query_response.get(entity_type, [])
            
            logger.info(f"Query returned {len(results)} {entity_type} results")
            return results
